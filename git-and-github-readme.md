...
1. --soft参数告诉Git重置HEAD到另外一个commit，但也到此为止。如果你指定--soft参数，Git将停止在那里而什么也不会根本变化。这意味着index,working copy都不会做任何变化，所有的在original HEAD和你重置到的那个commit之间的所有变更集都放在stage(index)区域中。
1. --hard参数将会blow out everything.它将重置HEAD返回到另外一个commit(取决于~12的参数），重置index以便反映HEAD的变化，并且重置working copy也使得其完全匹配起来。这是一个比较危险的动作，具有破坏性，数据因此可能会丢失！
1. --mixed是reset的默认参数，也就是当你不指定任何参数时的参数。它将重置HEAD到另外一个commit,并且重置index以便和HEAD相匹配，但是也到此为止。
...
* git rebase -i HEAD~10    : rebase 可以挤压(squash)多次commits到一起, 而不需要使用merge，这里挤压合并最后10次提交
...
# merge
git checkout dev
git merge feature # word.txt has conflict
git checkout --ours word.txt    # => keep contents of dev
git checkout --theirs word.txt  # => keep contents of feature

# rebase
git checkout dev
git rebase -i feature # word.txt has conflict
git checkout --ours word.txt    # => keep contents of feature
git checkout --theirs word.txt  # => keep contents of dev
```
...
1. 同一个产品开发的所有人员共享一个Repository，有一个trunk，单一Developer或是Developer团队可以有自己的private branch，所有修改最后都会回到主干; 
1. 只有在Release时才会有官方的分支，一般Developer不能对Release Branch作动作，只有Release Engineer可以更动Release Branch，当Release Branch完成它的任务，就会被砍掉; 
1. Bug先在trunk修好，之後把Commit合併到Release Branch，而不是在Release Branch修好再整合到trunk，這樣可以把修改Release Branch的人限制在最小程度。
